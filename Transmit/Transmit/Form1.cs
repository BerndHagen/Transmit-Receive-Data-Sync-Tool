...
namespace Client
{
    public partial class Form1 : Form
    {
        ...
        private readonly string _ipPlaceholderText = "Enter Server IP Address";
        ...
        // Displays the local IP address for reference
        private void DisplayLocalIP()
        {
            try
            {
                string localIP = GetLocalIPAddress();
                AddMessage($"Your IP: {localIP}");
            }
            catch (Exception ex)
            {
                AddMessage($"Could not determine local IP: {ex.Message}");
            }
        }
        ...
        // Sets placeholder text for both textboxes
        private void SetPlaceholders()
        {
            ...
            if (ip_txtbox != null && string.IsNullOrWhiteSpace(ip_txtbox.Text))
            {
                ip_txtbox.Text = _ipPlaceholderText;
                ip_txtbox.ForeColor = Color.Gray;
            }
        }
        ...
        private async void btnOpen_Click(object sender, EventArgs e)
        {
            ...
            {
                AddMessage("Please enter a valid server IP address!");
                return;
            }
            ...
            if (result == DialogResult.OK)
            {
                ...
                try
                {
                    await SendFileAsync(fileDlg.FileName, serverIP);
                }
                finally
                {
        ...
        // Gets and validates the server IP address from the textbox
        private string GetServerIP()
        {
            string ip = null;

            if (ip_txtbox.InvokeRequired)
            {
                ip_txtbox.Invoke(new Action(() =>
                {
                    ip = ip_txtbox.Text != _ipPlaceholderText ? ip_txtbox.Text.Trim() : null;
                }));
            }
            else
            {
            ...
            AddMessage($"Invalid IP address format: {ip}");
            return null;
        }

        // Asynchronously sends the selected file with error handling
        private async Task SendFileAsync(string filePath, string serverIP)
        {
            try
            {
                await SendFileWithHeaderAsync(filePath, serverIP);
            }
            catch (Exception ex)
            {
            ...
            if (ex is SocketException sockEx)
            {
                switch (sockEx.SocketErrorCode)
                {
                    ...
                    case SocketError.HostUnreachable:
                        return "Host is unreachable - check IP address";
                    default:
        ...
        private async Task SendFileWithHeaderAsync(string filePath, string serverIP)
        {
            ...
            AddMessage($"Connecting to server at {serverIP}:1234...");
            ...
            using (var tcpClient = new TcpClient())
            {
                ...
                var connectTask = tcpClient.ConnectAsync(serverAddress, 1234);
        ...
        private async Task WaitForConfirmationAsync(NetworkStream stream)
        {
            ...
            try
            {
                ...
                {
                    AddMessage("Server: No save directory selected. Transfer aborted.");
                }
                else
                {
        ...
        // Handles the IP textbox enter event to clear placeholder
        private void ip_txtbox_Enter(object sender, EventArgs e)
        {
            if (ip_txtbox.Text == _ipPlaceholderText)
            {
                ip_txtbox.Text = "";
                ip_txtbox.ForeColor = Color.Black;
            }
        }

        // Handles the IP textbox leave event to restore placeholder if empty
        private void ip_txtbox_Leave(object sender, EventArgs e)
        {
            if (string.IsNullOrWhiteSpace(ip_txtbox.Text))
            {
                ip_txtbox.Text = _ipPlaceholderText;
                ip_txtbox.ForeColor = Color.Gray;
            }
