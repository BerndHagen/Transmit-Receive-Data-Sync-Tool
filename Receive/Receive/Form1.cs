...
namespace Receive
{
    public partial class Form1 : Form
    {
        ...
        // Displays network information for clients to connect
        private void DisplayNetworkInfo()
        {
            try
            {
                string localIP = GetLocalIPAddress();
                UpdateStatus($"Server IP: {localIP}:{Port}");
                UpdateStatus($"Share this IP with clients to connect");
            }
            catch (Exception ex)
            {
                UpdateStatus($"Could not determine local IP: {ex.Message}");
            }
        }

        // Gets the local IP address for network connections
            ...
            try
            {
                ...
                using (NetworkStream networkStream = clientSocket.GetStream())
                {
                    ...
                    if (string.IsNullOrEmpty(savePath))
                    {
                        ...
                        await networkStream.WriteAsync(new byte[] { 0x03 }, 0, 1);
                        return;
                    }
